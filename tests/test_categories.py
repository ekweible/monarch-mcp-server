...
class TestUpdateCategory:
    ...
    async def test_update_name_and_icon(self, mock_get_client):
        ...
        result = json.loads(
            await update_category(
                category_id="cat-123", name="Fitness", icon="\U0001f3cb"
            )
        )
    ...
    async def test_dry_run_returns_current_and_proposed(self, mock_get_client):
        ...
        assert (
            mock_client.gql_call.call_args.kwargs["operation"] == "GetCategoryDetails"
        )
    ...
    async def test_dry_run_category_not_found(self, mock_get_client):
        ...
        result = json.loads(
            await update_category(category_id="nonexistent", name="Test", dry_run=True)
        )
    ...
    async def test_graphql_errors(self, mock_get_client):
        ...
        result = json.loads(
            await update_category(category_id="cat-123", name="x" * 500)
        )
